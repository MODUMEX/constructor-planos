...
/**
 * Mueve el PANEL divisor sobre su pilastra, sin tocar ninguna pieza.
 *
 * El panel se pega a la pilastra, y la pilastra es ancha —de 24 a 120 cm—, así
 * que el panel puede ir centrado en ella o corrido hacia un lado. Eso cambia el
 * claro libre de las dos cabinas vecinas, pero NO cambia ni la puerta ni la
 * pilastra: las piezas que se fabrican son exactamente las mismas y la tira
 * sigue midiendo lo mismo.
 *
 * El corrimiento no se guarda aparte: sale de comparar el ancho que tiene la
 * cabina contra el que le tocaría con el panel centrado.
 */
export function moverPanelEnPilastra(
  cabinas: Cabina[],
  ...
  gruesoCm: number,
  /** en vez de correrlo, dejarlo en este corrimiento; 0 es el centro */
  corridoDeseado?: number,
): Cabina[] {
  const n = cabinas.length
  ...
  const cuerpo = (c: Cabina) => (c.tipo === 'orinal' ? c.anchoCm : c.puerta.anchoCm)

  // lo que mediría cada cabina con el panel justo en el centro de la pilastra
  const centradaIzq = parteIzq(indice) + cuerpo(izq) + parteDer(indice)
  const centradaDer = parteIzq(indice + 1) + cuerpo(der) + parteDer(indice + 1)

  // el panel no puede salirse de la cara de la pilastra
  const tope = Math.max(0, anchoPil(indice + 1) / 2 - gruesoCm / 2)
  const actual = izq.anchoCm - centradaIzq
  const pedido = corridoDeseado ?? actual + deltaCm
  const corrido = Math.max(-tope, Math.min(tope, snap(pedido)))
  if (corrido === actual) return cabinas

  return cabinas.map((c, i) => {
    if (i === indice) return { ...c, anchoCm: centradaIzq + corrido }
    if (i === indice + 1) return { ...c, anchoCm: centradaDer - corrido }
    return c
  })
}
